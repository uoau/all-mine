...
const genData = (param) => {
    const {
        date, type, show, vm, isNowDays, view,
    } = param;
    let disabled = false;
    ...
    const dis = date.distance(vm.today, type);
    let isToday = dis == 0;
    if (view == 'quarter') {
        isToday = dis >= -2 && dis <= 0;
    }
...
export default {
                    ...
                    (event) => {
                        if (this.view == 'date' && !!this.value.start && !this.value.end) {
                            ...
                            if (target.tagName == 'SPAN') {
                                const string = target.getAttribute('string');
    ...
    methods: {
        ...
        chooseDate(d) {
            if (this.view == endView[this.type]) {
                ...
                const index = viewTypes.indexOf(this.view);
    ...
    computed: {
        ...
        weeks() {
            ...
            const days = weeks.splice(0, this.startWeek - 1);
            weeks.push(...days);
            return weeks;
        },
        months() {
        ...
        dates() {
            const nowDate = this.nowView;
            if (this.view == 'date') {
                const lastdayofmonth = nowDate.endOf(manba.MONTH);
                const firstdayofmonth = nowDate.startOf(manba.MONTH);
                ...
                const lastdayoflastmonth = firstdayofmonth.add(-1, manba.DAY);
                const dates = [];
                const lastMonthDays = lastdayoflastmonth.date() - startDay;
                ...
                const nextMonth = lastdayofmonth.add(1, manba.DAY);
                const nextMonthDays = 7 * 6 - dates.length;
